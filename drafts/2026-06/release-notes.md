...
Facility filtering, oldest-date indicators, open order counts, unfillable metrics, fulfillment progress, and facility rejection APIs let the dashboard become a launch point into the exact queue that needs attention, not just a report that has to be interpreted somewhere else.
...
### Customer 360 for service and order context
Order Manager also introduces a new Customer 360 view. The customer profile is intentionally bounded: the primary customer record loads separately from orders, returns, communications, relationships, and open tasks, so the page can show a complete service picture without forcing every history list into one overwhelming view.

That gives customer service teams one customer workspace instead of a trail of order-only screens. A rep can start from the customer, review contact and relationship data, see customer-linked orders with hold tasks, and connect the customer back to order detail context such as communications, returns, work efforts, and party information. Customer search also received Solr indexing support, so users can find the customers with flexible keywords like email and phone number.

*Sources: [oms#594](https://github.com/hotwax/oms/pull/594), [oms#648](https://github.com/hotwax/oms/pull/648), [oms#660](https://github.com/hotwax/oms/pull/660), [oms#688](https://github.com/hotwax/oms/pull/688), [oms#641](https://github.com/hotwax/oms/pull/641), [oms#642](https://github.com/hotwax/oms/pull/642), [oms#649](https://github.com/hotwax/oms/pull/649)*

## Products and sourcing

### Products as the operational product data app
The Products app introduces a stronger backend foundation for managing the product data operations teams actually work with. The June release adds APIs for products, variants, features, good identifications, product associations, Shopify shop products, product facilities, product-facility configuration, and inventory-facing product views.
...
HotWax now has a native integration for Shopify fulfillment holds and third-party fulfillment services. Shopify-assigned fulfillment service work can be respected instead of re-brokered, OMS-assigned 3PL work can still be routed by HotWax, and Shopify fulfillment holds can pause order processing until Shopify releases the work.
...
### Shopify location and inventory sync
The setup flow for new facilties and accounts all together has been simplified with the new facilities and inventory integartions. HotWax can fetch Shopify locations, create OMS facilities from those locations, queue order history sync and start an initial inventory reset to get up an running in minutes.

*Sources: [hotwax-shopify-oms-bridge#227](https://github.com/hotwax/hotwax-shopify-oms-bridge/pull/227), [hotwax-shopify-oms-bridge#255](https://github.com/hotwax/hotwax-shopify-oms-bridge/pull/255), [hotwax-shopify-oms-bridge#271](https://github.com/hotwax/hotwax-shopify-oms-bridge/pull/271), [hotwax-shopify-oms-bridge#277](https://github.com/hotwax/hotwax-shopify-oms-bridge/pull/277), [hotwax-shopify-oms-bridge#278](https://github.com/hotwax/hotwax-shopify-oms-bridge/pull/278), [hotwax-shopify-oms-bridge#281](https://github.com/hotwax/hotwax-shopify-oms-bridge/pull/281), [hotwax-poorti#238](https://github.com/hotwax/hotwax-poorti/pull/238), [hotwax-poorti#282](https://github.com/hotwax/hotwax-poorti/pull/282), [hotwax-poorti#283](https://github.com/hotwax/hotwax-poorti/pull/283), [mantle-shopify-connector#344](https://github.com/hotwax/mantle-shopify-connector/pull/344), [mantle-shopify-connector#356](https://github.com/hotwax/mantle-shopify-connector/pull/356)*

### Native pickup foundation
Shopify native BOPIS fulfillment orders give HotWax a cleaner foundation for pickup demand. The app direction is to use Shopify Fulfillment Orders as the source of truth for pickup work.

*Sources: Shopify Native BOPIS Fulfillment Order Integration PR FAQ draft*

### Shipping gateway connection pooling
Shipping gateway calls now reuse a pooled HTTP client instead of creating a separate client for each request. Carrier calls to services such as UPS and FedEx can keep connections alive per destination, reducing connection overhead during rate, label, void, and validation requests.

*Sources: [hotwax-oms#525](https://github.com/hotwax/hotwax-oms/pull/525)*

## Agent and admin foundations

### Agent Composer and Workforce
Agent Composer introduces the ability to build governed OMS agents with approved HotWax capabilities. Users can define an agent, improve its instructions, select a model, choose tools, decide which actions require approval, preview behavior, and activate the agent for use in Workforce.

Agents are not a sidecar chatbot. They become governed OMS workers that can inspect order, inventory, fulfillment, routing, product, and integration context through approved tools, while sensitive actions remain visible and approval-driven.
